...
class Dashboard:
    def __init__(self):
        ...
        self.s3_config = {
            ...
            "files": {
                "revenue": "demo-bucket-d1/KPI_Revenue_total_counts.csv",
                "customers": "demo-bucket-d1/customers_6months.csv",
                "subscriptions": "demo-bucket-d1/subscriptions_6months.csv",
                "payment": "demo-bucket-d1/payments_outcome_data.csv",
                "financial": "demo-bucket-d1/financial.csv",
                "customer_meta":"demo-bucket-d1/customers_metadata.csv",
            }}
    ...
    def Summary(self):
        ...
        payment_df.columns = payment_df.columns.str.replace(r'charge_', '')
        ...
        #New things
        #Total Active Sub
        total_active_sub = merged_df[merged_df["status"].str.contains("active|trialing|past_due")].shape[0]
        ...
        total_users = merged_df['cust_id'].nunique()
        total_trial_subscriptions = merged_df[merged_df['status'] == 'trialing']['cust_id'].nunique()
        total_paid_subscriptions = merged_df[merged_df['status'] == 'active']['cust_id'].nunique()
        

        #total_users = subscriptions_df['customer_id'].nunique()
        #total_trial_subscriptions = subscriptions_df[subscriptions_df['status'] == 'trialing']['id'].nunique()
        #total_paid_subscriptions = subscriptions_df[subscriptions_df['status'] == 'active']['id'].nunique()
        
        #merged_df = pd.merge(revenue_df, payment_df, on='customer_id', how='left').merge(subscriptions_df, on='customer_id', how='left')
        total_products = revenue_df['description'].str.contains('Headphones|Chair').count()
        total_accessories = revenue_df['description'].str.contains('Charging Cable|Replacement Battery|Accessory Shipping').sum()
        total_payments_failed = payment_df[payment_df['status'] == 'failed']['id'].nunique()


        subscriptions_df['created'] = pd.to_datetime(subscriptions_df['created'], errors='coerce')
        today = subscriptions_df['created'].dt.date.max()
        ...
        new_subscriptions_df= subscriptions_df[subscriptions_df["status"].str.contains("trialing|active")]
        new_sub_today = new_subscriptions_df[new_subscriptions_df['created'].dt.date == today]['id'].nunique()
        new_sub_last7days = new_subscriptions_df[(new_subscriptions_df['created'].dt.date >= last7days) & (new_subscriptions_df['created'].dt.date < today)]['id'].nunique()
        new_sub_last30days = new_subscriptions_df[(new_subscriptions_df['created'].dt.date >= last30days) & (new_subscriptions_df['created'].dt.date < today)]['id'].nunique()
        new_sub_1_year = new_subscriptions_df[(new_subscriptions_df['created'].dt.date >= last_1_year) & (new_subscriptions_df['created'].dt.date < today)]['id'].nunique()


        total1,total2,total3 = st.columns(3, gap='small')
        with total1 :
            st.info('Total Users')
            #st.metric(label="", value='52114')
            st.metric(label="", value= f" {total_users:,.0f}")
        with total2 :   
            st.info('Total Trial Subscriptions')
            st.metric(label="", value= f" {total_trial_subscriptions:,.0f}")
        with total3 :
            st.info('Total Paid Subscriptions')
            st.metric(label="", value= f" {total_paid_subscriptions:,.0f}")

        total1,total2 = st.columns(2, gap='small')
        with total1 :
            st.info('Total Monthly Subscriptions')
            st.metric(label="", value= f" {new_sub_last30days:,.0f}")
        with total2 :   
            st.info('Total Yearly Subscriptions')
            st.metric(label="", value= f" {new_sub_1_year:,.0f}")
        #with total3 :
        #    st.info('Total Partners on Trial')
        #    st.metric(label="", value='3,814')
        #with total4:
        #    st.info('Total Partners Monthly paid')
        #    st.metric(label="", value=' 5,358')
        ...
        with total4:
            st.info('Last 1 Year')
            st.metric(label="", value='18668')
        
        st.subheader("Subscriptions Renewed")
        total1,total2,total3,total4 = st.columns(4, gap='small')
        with total1 :
            st.info('Last 7 Days')
            st.metric(label="", value='1231')
        with total2 :   
            st.info('Last 15 Days')
            st.metric(label="", value='2626')
        with total3 :
            st.info('Last 1 Month')
            st.metric(label="", value=' 5,328')
        with total4:
            st.info('Last 1 Year')
            st.metric(label="", value='19132')

        total1,total2, total3 = st.columns(3, gap='small')
        with total1 :
            st.info('Total Products sold')
            st.metric(label="", value= f" {total_products:,.0f}")
        with total2 :
            st.info('Total Accessories sold')
            st.metric(label="", value= f" {total_accessories:,.0f}")
        with total3:
            st.info('No of Payment Failures')
            st.metric(label="", value= f" {total_payments_failed:,.0f}")
        ...
        new_users_1_year = customers_df[(customers_df['created'].dt.date >= last_1_year) & (customers_df['created'].dt.date < today)]['id'].nunique()
    ...
    def Revenue(self):
        ...
        filtered_df_search = filtered_df[(filtered_df['email'].str.contains(search_term, case=False, na=False))]
    ...
    def Customers(self):
        ...
        end_date = st.sidebar.date_input("End date", subscriptions_df['trial_end'].max().date())
        start_date = pd.to_datetime(start_date)
        end_date = pd.to_datetime(end_date)
        ...
        filtered_cust_sub_df_total = subscriptions_df.merge(customers_df, left_on="customer_id", right_on="id", how="inner")


        # Calculate the total number of active, inactive, trialing, past due, paused, and incomplete expired subscriptions
        total_active = filtered_cust_sub_df_total[filtered_cust_sub_df_total["status"] == "active"].shape[0] # Calculate the total number of active customers
        total_inactive = filtered_cust_sub_df_total[filtered_cust_sub_df_total["status"] != "active"].shape[0] # Calculate the total number of inactive customers
        total_trialing = filtered_cust_sub_df_total[filtered_cust_sub_df_total["status"] == "trialing"].shape[0] # Calculate the total number of trialing customers
    ...
    def Subscriptions(self):
        ...
        customers_df = self.load_data_from_s3('customers')
        revenue_df = self.load_data_from_s3('revenue')
        customer_meta_df = self.load_data_from_s3('customer_meta')
        ...
        #customer_meta_df = pd.read_csv("customers_metadata.csv")
        st.subheader("Revenue by Source")

        # Key Filter
        keys = customer_meta_df['key'].unique()
        selected_key = st.selectbox("Select Key", keys)
        
        # Filter Data
        filtered_data = customer_meta_df[customer_meta_df['key'] == selected_key]
        ...
        # Streamlit App
        st.title("Subscription Trends Analysis")
    ...
    def Payment(self):
        ...
        st.sidebar.header("Select Date Range:")
        payment_df['created'] = pd.to_datetime(payment_df['created'], errors='coerce')

        start_date = st.sidebar.date_input("Start date", payment_df["created"].min().date())

        end_date = st.sidebar.date_input("End date", payment_df["created"].max().date())

        # Filter data
        filtered_df  = payment_df[(payment_df['created'] >= pd.to_datetime(start_date)) & (payment_df['created'] <= pd.to_datetime(end_date))]
        
        titles = filtered_df['payment_method_type'].unique()
        titles = np.insert(titles, 0, 'All')
        st.sidebar.header("Select Payment Method:")
        title = st.sidebar.selectbox("Payment Method", titles, index=0)
        if title != 'All':
            filtered_df = filtered_df[filtered_df['payment_method_type'] == title]
            
        search_term = st.text_input("Search by payment method :")

        # Filter data based on the search term
        filtered_df_search = filtered_df[
            (filtered_df['payment_method_type'].str.contains(search_term, case=False, na=False)) 
        ]
        # Display data
        with st.expander("VIEW DATA"):
            filtered_df['created'] = pd.to_datetime(filtered_df['created']).dt.date
            showData = st.multiselect('Filter: ', filtered_df.columns, default=['id', 'amount', 'amount_refunded', "payment_method_type", 
                                                                                'calculated_statement_descriptor',  'created', 
                                                                                'description', 'status'])
            st.dataframe(filtered_df_search[showData], use_container_width=True)

        total_transactions_total = payment_df.shape[0] # Calculate the total number of transactions
        successful_transactions_total = payment_df[payment_df["status"] == "succeeded"].shape[0] # Calculate the number of successful transactions
        failed_transactions_total = payment_df[payment_df["status"] == "failed"].shape[0] # Calculate the number of failed transactions
        ...
        with total1:
            ...
            st.metric(label="Total Transactions", value= f" {total_transactions_total:,.0f}") #f" {total_transactions:,.0f}"

        with total2:
            st.info('Number of successful transactions')
            st.metric(label="Number of successful transactions:", value=f"{successful_transactions_total:,.0f}") #f"{successful_transactions:,.0f}"

        with total3:
            st.info('Number of failed transactions')
            st.metric(label="Number of failed transactions:", value=f"{failed_transactions_total:,.0f}") # f"{failed_transactions:,.0f}"
        
        st.subheader("Payments - selected period")
        total1, total2, total3 = st.columns(3, gap='small')
        with total1:
            st.info('No of  Transactions')
            st.metric(label="", value= f" {total_transactions:,.0f}") 

        with total2:
            st.info('No of successful transactions')
            st.metric(label="", value=f"{successful_transactions:,.0f}") #f"{successful_transactions:,.0f}"

        with total3:
            st.info('No of failed transactions')
            st.metric(label="", value=f"{failed_transactions:,.0f}") # f"{failed_transactions:,.0f}"
        st.markdown("---")
